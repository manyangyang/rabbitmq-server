...
configure_lager() ->
    ...
    SinkConfigs = lists:map(
        ...
        proplists:get_value(categories, application:get_env(rabbit, log, []), [])),
...
generate_handler(Backend, HandlerConfig) ->
    [{Backend,
        lists:ukeymerge(1, lists:usort(HandlerConfig),
                           lists:usort(default_handler_config(Backend)))}].
...
generate_lager_sinks(SinkNames, SinkConfigs) ->
    lists:map(fun(SinkName) ->
        ...
        SinkHandlersConfig = lists:keydelete(level, 1, SinkConfig),
        Handlers = case generate_lager_handlers(SinkHandlersConfig) of
            %% No handlers defined
            [] ->
